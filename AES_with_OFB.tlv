   ...
   |encrypt
      @0
         \viz_js
            ...
            init() {
               this.row_colors = ["red", "blue", "#00ff00", "#ff00ff"];
         ...
         /subbytes
            /sub_word[*]
               ...
               /sbox_subword[*]
                  $word_byte[7:0] = /sub_word$word[32 - 8 * (1 + #sbox_subword) +: 8];
                  $sb_out_byte[7:0] = /sub_word$sb_out[32 - 8 * (1 + #sbox_subword) +: 8];
                  $ssr_out_byte[7:0] = /subbytes$ssr_out[(15 - ((#sub_word * 4) + #sbox_subword)) +: 8];
                  \viz_js
                     box: {strokeWidth: 0},
                     ...
                     init() {
                        ...
                        text = new fabric.Text("--\n--\n--", {originX: "center", originY: "center", textAlign: "center", left: 25, top: 25, fontFamily: "mono", fontSize: 12.5})
                        return {circ, text}
                     },
                     onTraceData() {
                        this.getObjects().circ.set({stroke: this.getScope("encrypt").context.row_colors[this.getIndex("sbox_subword")]})
                     },
                     render() {
                        if ('|encrypt$valid_blk'.asBool()) {
                           this.getObjects().text.set({
                              text: `${'$word_byte'.asInt().toString(16)}\n${'$sb_out_byte'.asInt().toString(16)}\n${'$ssr_out_byte'.asInt().toString(16)}`
                           })
                        }
                        return [];
                     }
               ...
               \viz_js
                  ...
                  init() {
                     img = this.newImageFromURL(
                        ...
                        "Matt Crypto -- CC0",
                        {left: 0, top: 0, width: 300, fill: "#30483c", strokeWidth: 1,},
                        {strokeWidth: 0}
                     )
                     return {img}
                  },
                  where: {left: 460, top: 95}
               /ax[3:0]
                  /ay[3:0]
                     ...
                     \viz_js
                        ...
                        layout: "vertical",
                        init() {
                           circ = new fabric.Circle({fill: "#FFFD", radius: 13})
                           text = new fabric.Text("--", {originX: "center", originY: "center", textAlign: "center", left: 13, top: 13, fontFamily: "mono", fontSize: 12.5})
                           return {circ, text}
                        },
                        render() {
                           if ('|encrypt$valid_blk'.asBool()) {
                  ...
                  /by[3:0]
                     ...
                     \viz_js
                        ...
                        render() {
                           if ('|encrypt$valid_blk'.asBool()) {
                              this.getObjects().text.set({
                                 text: `${'$sb_out_byte'.asInt().toString(16)}`
                              })
                           }
                           return [];
                        },
                        where: {left: 191, top: 6}
         /shift_row
            /map
               \viz_js
                  box: {strokeWidth: 0},
                  init() {
                     img = this.newImageFromURL(
                        ...
                        "Matt Crypto -- CC0",
                        {left: -30, top: 4, width: 331, fill: "#30483c", strokeWidth: 1,},
                        {strokeWidth: 0}
                     )
                     return {img}
                  },
                  where: {left: 430, top: 285}
               /ax[3:0]
                  /ay[3:0]
               ...
               /bx[3:0]
                  /by[3:0]
                     $ANY = |encrypt/subbytes/sub_word[#bx]/sbox_subword[#by]$ANY;
                     \viz_js
                        box: {strokeWidth: 0},
                        layout: "vertical",
                        init() {
         ...
         /mixcolumn
            /xx[*]
               ...
               /yy[*]
                  $ssr_out_byte[7:0] = |encrypt/subbytes/sub_word[#xx]/sbox_subword[#yy]$ssr_out_byte;
                  // $ss, $cc, $oo
                  \viz_js
                     box: {strokeWidth: 0},
                     layout: {left: 56, top: 32},
                     init() {
                        ...
                        text = new fabric.Text("--\n--\n--\n--", {originX: "center", originY: "center", textAlign: "center", left: 25, top: 25, fontFamily: "mono", fontSize: 9})
                        return {circ, text}
                     },
                     onTraceData() {
                        this.getObjects().circ.set({stroke: this.getScope("encrypt").context.row_colors[this.getIndex("yy")]})
                     },
                     render() {
                        if ('|encrypt$valid_blk'.asBool()) {
                           this.getObjects().text.set({
                              text: `${'$ssr_out_byte'.asInt().toString(16)}\n${'$ss'.asInt().toString(16)}\n${'$cc'.asInt().toString(16)}\n${'$oo'.asInt().toString(16)}`
                           })
                        }
                        return [];
                     }
               ...
               \viz_js
                  ...
                  init() {
                     img = this.newImageFromURL(
                        ...
                        "Matt Crypto -- CC0",
                        {left: 1, top: -14, width: 300, fill: "#30483c", strokeWidth: 1,},
                        {strokeWidth: 0}
                     )
                     return {img}
                  },
                  where: {left: 460, top: 455}
               /ax[3:0]
                  /ay[3:0]
